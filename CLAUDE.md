...
| `aitcc app register` | ✅ (코드 + 테스트) | YAML/JSON 매니페스트 + 이미지 업로드 + 등록 submit. Submit payload shape는 dog-food (#23) 로 **확정**되어 flat top-level 구조. **단, review trigger 는 별도 endpoint 로 추정되며 미구현** — 현 submit 은 `reviewState: null` 로 skeleton app 만 만든다. |
...
**Submit payload shape 는 dog-food (#23, 2026-04-22) 로 확정**: 서버는 flat top-level 문서를 파싱하며, 그 shape 는 persisted `app ls` row 와 동일하다. 이전 번들 분석에서 추정했던 `{miniApp, impression}` wrapper 는 **top-level key 외 전부 silently drop** 했던 것이 dog-food 에서 드러났다. 교정이 필요하면 `src/api/mini-apps.ts` 의 `MiniAppSubmitPayload` 와 `src/commands/register-payload.ts` 의 transform 만 고치면 된다 — 매니페스트 shape 은 user-facing 계약이므로 변경하지 않는다. 캡처 원본: 움브렐라 `.playwright-mcp/FORM-SCHEMA-CAPTURED.md` + `dogfood-23-app-ls-after.json`.

**여전히 inferred**: `/mini-app/review` 가 `reviewState: null` 을 돌려주는 걸 보면 이 endpoint 는 skeleton app 만 만들고 실제 검토 요청은 **별도 endpoint** 로 트리거되는 것으로 보인다. 다음 dog-food 에서 해당 endpoint 를 캡처해 `aitcc app review-request` 서브커맨드로 추가 예정.
...
titleEn: SDK Reference          # 영어 앱 이름 (영문/숫자/공백/콜론(:)만 허용 — dog-food #23 서버 규칙)
...
description: |-
  상세 설명 (멀티라인 YAML OK, <= 500 code points — dog-food #23 서버 규칙)
...
**서버 validation 규칙** (로컬 pre-check 로 잡아 agent-plugin 이 `invalid-config` 로 받게 함):

- `titleEn` charset: `^[A-Za-z0-9 :]+$`. 위반 시 서버가 HTTP 400 `errorCode=4000` "앱 영문 이름은 영어, 숫자, 공백, 콜론(:)만 사용 가능해요" 로 거부.
- `description` (= server-side `detailDescription`) 길이: **code point 기준 <= 500**. 위반 시 "앱 상세설명은 최대 500자를 넘어갈 수 없어요". Code point count (`[...str].length`) 로 세는 것은 서버 내부 카운팅 규칙이 불명확해 strict side 로 에러 메시지를 내기 위함.
...
6. `POST /workspaces/:wid/mini-app/review` 로 flat payload 제출. 응답은 `{appId, reviewState: null}` — dog-food 결과 이 endpoint 는 skeleton app 생성만 수행하며, **별도 review-trigger endpoint** 가 존재하는 것으로 추정 (미구현).
...
**TODO 이후 (dog-food task)**: 검토 요청 (`검토 요청하기` 버튼에 해당하는 별도 호출) endpoint 는 아직 미캡처. dog-food #23 응답의 `reviewState: null` 은 현 `/mini-app/review` endpoint 가 skeleton 생성만 수행한다는 강한 신호 — 실제 trigger 는 별도 endpoint 로 추정되며 다음 dog-food 에서 확정 후 `aitcc app review-request` subcommand 로 추가한다. 실제 payload 에 동의 체크박스 필드가 있는지도 미확인 (현 shape 에는 없음).
