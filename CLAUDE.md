...
| `aitcc app register` | ✅ (코드 + 테스트) | YAML/JSON 매니페스트 + 이미지 업로드 + 등록 submit. **submit payload shape는 bundle 분석 기반 inferred** — dog-food task #23에서 실제 네트워크로 확인/교정. |
...
**주의: submit payload shape 는 정적 번들 분석으로 추정한 값이다.** 콘솔 UI 는 중간 draft 를 서버로 왕복하지 않아서 실제 요청 본문을 캡처할 수 없었다. `apps-in-toss-community/.playwright-mcp/VALIDATION-RULES.md` 와 `FORM-SCHEMA-CAPTURED.md` 의 bundle 추출물을 기반으로 구성했으며, dog-food task #23 (sdk-example 실등록) 에서 처음으로 실제 네트워크 응답이 관측된다. 교정이 필요하면 `src/api/mini-apps.ts` 의 `MiniAppSubmitPayload` 와 `src/commands/register-payload.ts` 의 transform 만 고치면 된다 — 매니페스트 shape 은 user-facing 계약이므로 변경하지 않는다.
