...
| `aitcc app deploy <path> --app <id>` | ✅ | One-shot 배포 래퍼. `bundles upload` (+ optional `review` + optional `release`)를 하나의 명령으로 연결. `--deployment-id` 생략 시 `.ait` 내부 `app.json._metadata.deploymentId`를 `fflate`로 추출 (`src/config/ait-bundle.ts`). 스텝은 opt-in: `--request-review --release-notes <text>`로 review 추가, `--release --confirm`으로 release 추가. `--release`는 bundle이 이미 APPROVED일 때만 동작 — 일반적으로 두 번째 실행에서 사용. `--dry-run --json`은 네트워크 없이 plan만 출력 (`{steps, deploymentId, bytes, ...}`). Partial failure 시 `uploaded`/`reviewed`/`released` 플래그를 JSON에 포함해 agent-plugin이 재시도할 스텝을 판단. |
