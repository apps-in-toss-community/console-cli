...
`apps-in-toss-community`는 **비공식(unofficial) 오픈소스 커뮤니티** — "공식/토스가 제공하는/powered by Toss" 표현 금지. 상세는 umbrella [`CLAUDE.md`](https://github.com/apps-in-toss-community/umbrella/blob/main/CLAUDE.md) "프로젝트 성격" 참조.
...
이 repo와 직접 관련된 짝만 — 전체 그림은 umbrella [`CLAUDE.md`](https://github.com/apps-in-toss-community/umbrella/blob/main/CLAUDE.md)의 "짝(pair) 관계" 참조.
...
**Next** (umbrella [`TODO.md`](https://github.com/apps-in-toss-community/umbrella/blob/main/TODO.md)의 `console-cli` 섹션): `app logs` (deferred — 아래 "App runtime logs" 참조), 그 외 backlog 항목.
...
- **`app register` submit shape** — `POST /workspaces/:wid/mini-app/review`에 `{miniApp, impression}` nested wrapper + `impression.categoryIds: [number]`. "등록 + 심사 제출 원샷" — 별도 update/review-trigger 엔드포인트 없음. 0.1.7에서 "flat + `categoryList: [{id}]`"로 regression한 적 있는데, `GET /mini-app/:id`가 **current view만** 반환하는 걸 draft 소실로 오해한 결과 (실제론 `/with-draft`로 읽으면 정상 persist). 상세 + 캡처: [`docs/api/mini-apps.md`](./docs/api/mini-apps.md) "submit" 항목 + "Drift history".
...
공통 부분(Node 24, pnpm 10.33.0, TypeScript strict, Biome, Changesets, pre-commit hook 등)은 umbrella [`CLAUDE.md`](https://github.com/apps-in-toss-community/umbrella/blob/main/CLAUDE.md)의 "공통 스택" 참조. 이 repo 고유:
...
배포 정책 일반(Type A 매트릭스, 버전 정책, Changesets 흐름, Version Packages PR 타이밍)은 umbrella [`CLAUDE.md`](https://github.com/apps-in-toss-community/umbrella/blob/main/CLAUDE.md) "배포 전략" 및 [`meta/release-strategy.md`](https://github.com/apps-in-toss-community/umbrella/blob/main/meta/release-strategy.md) 참조. 이 repo 고유:
...
- 엣지 케이스 (umbrella [`TODO.md`](https://github.com/apps-in-toss-community/umbrella/blob/main/TODO.md) `console-cli` 섹션): `shasum`/`sha256sum` fallback, `$HOME` 없을 때 `/tmp` fallback, release asset 업로드 레이스에 exp-backoff 30s 재시도, 기존 root 소유 바이너리 감지, `AITCC_QUIET=1`.
...
정식 Apple notarization (Developer Program $99/년)은 1.0 item. Bun 업스트림이 stub 생성을 고치면 rcodesign 의존성 제거 가능 — umbrella [`TODO.md`](https://github.com/apps-in-toss-community/umbrella/blob/main/TODO.md) backlog.
