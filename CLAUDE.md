...
App-scoped 명령(`app status`, `app deploy`, `app certs ls` 등)이 매번 `--workspace`/`<appId>`를 받지 않아도 되도록, cwd → 가장 가까운 git repo root까지 ancestor walk하며 `aitcc.yaml`/`aitcc.json`을 찾고 거기서 `workspaceId`/`miniAppId`를 읽는다. 우선순위(높은 것부터):

- **workspace**: `--workspace` flag → `AITCC_WORKSPACE` env → yaml `workspaceId` → session `currentWorkspaceId`
- **miniApp**: positional/flag `<appId>` → `AITCC_APP` env → yaml `miniAppId`

`--workspace`로 workspace를 명시 override하면 yaml의 `miniAppId`는 invalidate (다른 workspace 소속일 수 있음). env 경로의 override는 yaml miniApp을 유지 — env는 "이 workspace로 한동안 작업한다"는 의도, flag는 "이 명령 한 번만"이라는 의도로 분리한다.
...
- **`GET /mini-app/:id` single-snapshot envelope (issue #219, 2026-07-23~)** — 구 `/with-draft` (`approvalType` + `current` + `rejectedMessage` + `draft` 조합으로 client-side derive)는 **404**. 대체 경로는 plain `GET /mini-app/:id` — 응답이 `{miniApp, hasApproved, hasInReview, hasDraft, isBeforeFirstReview, approvalType, rejectedMessage}`로 확장됐고, `approvalType`/`rejectedMessage`는 `miniApp`의 **형제 필드**다(안에 nest된 게 아님 — 착각하기 쉬운 지점, 검증 단계 2차 실측에서 잡힘). `app status`는 이 새 shape에서 동일 로직으로 derive. 서버 권위 상태가 필요하면 `app service-status` (`/review-status`). [`docs/api/mini-apps.md`](./docs/api/mini-apps.md) 새 `GET /mini-app/:id` 항목 + "Drift history" 5번.
...
**첫 번째 매니페스트 작성**: `aitcc app init`이 인터랙티브 prompt로 필수 필드를 받아 cwd에 well-formed `aitcc.yaml`을 떨어뜨린다. workspace는 API에서 fetch한 목록에서 select하고, optional 필드(`homePageUri`/`logoDarkMode`/`keywords`/`horizontalScreenshots`)는 주석 처리된 줄로 미리 깔린다. 이미지(logo, thumbnail, screenshots)는 placeholder 경로만 적고 사용자가 `./assets/`에 넣는다 — dimension 검증은 register 시점에. prompt validators는 `src/config/app-manifest.ts`가 export하는 한 vault의 limit/regex (`MANIFEST_LIMITS`, `APP_NAME_REGEX`, `TITLE_KO_REGEX`, `TITLE_EN_REGEX`, `isValidEmail`, `isTitleCaseWord`, `countCodepointsExcludingSpaces`)를 그대로 import해 동일 규칙을 두 곳에서 유지하는 drift를 막는다. 인터랙티브 명령이라 `--json`/non-TTY는 `interactive-required`로 거부한다 (exit 2). 구현은 `src/commands/app-init.ts`, render-only 모듈은 `src/commands/app-init-template.ts`.
...
조직의 다른 저장소는 [GitHub organization](https://github.com/apps-in-toss-community) 참고. (`aitc.dev` 도메인은 종료되므로 그 호스트를 가리키는 링크는 새로 만들지 않는다.)
