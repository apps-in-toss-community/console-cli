...
- `aitcc workspace show [--workspace <id>]` — dump the workspace detail (business registration / verification / review state). `--json` surfaces the full payload under `extra`.
- `--json` is supported on every subcommand and follows the existing exit-code contract (`ok`, `authenticated`, `reason`).
